...
E-ANSWER-02 uses computed visibility, viewport coverage and center-point occlusion
when browser geometry is available. Raw class/role matching is only a lower-confidence
fallback for imported observations. E-ANSWER-04 uses measured paragraph position
when available; its character-order fallback has medium confidence and is explicitly
labelled as unverified visual position. E-ORIENT-01 includes visible header/body text
in its early-text sample; page topics are no longer fed back as brand aliases.

E-CONTINUE-03 accepts a route in navigation as well as main content and recognizes
the observed hub's canonical URL. It does not infer arbitrary index-file equivalence
without canonical evidence. Nested utility endpoints are excluded.
