...
from lib.common.extract import extract_metadata, language_supported
    ...
    for url, page in effective_pages(store).items():
        ...
        findings.append(
            make_finding(
                ...
                evidence=f"Checked title \"{title}\", logo alt text, and first 800 body-text characters on {url}; visual viewport not measured for this check",
...
def check_e_orient_03(store: Dict[str, Any]) -> List[Dict[str, Any]]:
    if store.get('archetype') in {'local-business', 'personal-portfolio', 'web-app'}:
        return []
...
def check_e_answer_02(store: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for url, page in rendered_pages_only(store).items():
        geometry = page.get('measurements')
        overlay = (geometry.get('blocking_overlay') if geometry is not None
                   else find_blocking_overlay(page["html"]))
        if not overlay:
            continue
        findings.append(
            make_finding(
...
def check_e_answer_04(store: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for url, page in rendered_pages_only(store).items():
        ...
        for question in probe.get("value", {}).get("questions", []):
            ...
            geometry = page.get("measurements") or {}
            blocks = [b for b in geometry.get("blocks", []) if span in b.get("text", "")]
            below_viewport = bool(blocks) and all(b.get("top", 0) >= geometry.get("viewport_height", float("inf")) for b in blocks)
            buried = below_viewport if blocks else fraction > 0.5
            if not buried and not collapsed:
                continue
            ...
            findings.append(
                make_finding(
                    ...
                    severity="high",
                    confidence="high" if blocks or collapsed else "medium",
                    ...
                    observed_signal=(
                        "answer is inside a collapsed region" if collapsed else
                        "answer measured below the initial viewport" if below_viewport else
                        f"text-order fraction {fraction:.2f} (> 0.50); visual position unverified"
                    ),
                    ...
                    suggested_action={
                        ...
                        "validation": "Re-render; the same evidence span is visible near the start of the content and outside any collapsed region.",
                    },
                )
            )
    return findings
...
def check_e_continue_03(store: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for url, page in pages.items():
        ...
        from lib.common.extract import extract_links, extract_canonical_url
        links = extract_links(page["html"], url)
        hrefs = {link["href"].split('#', 1)[0].rstrip("/") for link in links}
        hub_aliases = {hub.rstrip('/'), extract_canonical_url(pages[hub]["html"], hub).rstrip('/')}
        if hub_aliases & hrefs:
            continue
