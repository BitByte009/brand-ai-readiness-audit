...
| D-CRAWL-04 | A 5xx with no independent repeat-confirmation evidence | never fires |
...
| D-CRAWL-09 | No independent confirmation of persistence for a block | never fires (rate limiting we caused, not a block) |
...
mismatch or mojibake, each independently gated), D-CRAWL-09 (initial block + independent
confirmation), D-CRAWL-14 (parameter ratio + budget exhaustion).

The live collector never retries 429/503: it stops and records limited coverage.
A retry field in a detector observation contract is not permission to issue retries.
Missing sitemap observations and unmeasured entry statuses are unknown. Local DNS,
policy and redirect failures cannot alone establish D-CRAWL-15.
