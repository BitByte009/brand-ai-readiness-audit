    ...
    for url, fetch, render in _paired_pages(store):
        ...
        for question in probe.get("value", {}).get("questions", []):
            ...
            if not span:
                continue
            rendered_text = _main_text(render['value'].get('html', ''))
            if span not in raw_text and span in rendered_text and question.get('relevant'):
                findings.append(
                    make_finding(
...
def check_d_render_03(store: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for url, fetch, render in _paired_pages(store):
        ...
        rendered_only = sorted(link for link in rendered_links - raw_links
...
def check_d_render_05(store: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for url, fetch, render in _paired_pages(store):
        ...
        # A finite complete listing needs no next page. Require an observed
        # continuation control before inferring content is hidden behind it.
        more = soup.select('[data-next-page], [data-has-more="true"]')
        more += [b for b in soup.find_all('button') if re.search(
            r'\b(load|show)\s+more\b', b.get_text(' ', strip=True), re.I)
                 and not b.has_attr('disabled') and not b.has_attr('hidden')]
        if not more:
            continue
