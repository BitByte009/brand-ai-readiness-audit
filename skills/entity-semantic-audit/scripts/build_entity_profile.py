        ...
        for node in jsonld_nodes_of_type(html, IDENTITY_NODE_TYPES):
            ...
            for alias in aliases if isinstance(aliases, list) else [aliases]:
                if isinstance(alias, str) and alias.strip():
                    candidates.append({"value": alias.strip(), "source": "alias", "source_url": url, "observation_id": oid})
            if node.get("name"):
                candidates.append(
...
def _structured_identity(pages: Dict[str, Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    """Return a repeated, visible, same-site structured identity when one exists."""
    nodes = []
    for url, page in pages.items():
    ...
    visible = any(
        normalized in normalize_name(
            extract_metadata(page["html"])["title"] + " " + extract_text(page["html"])
        )
...
def _canonical_name_field(
    ...
    candidates = [c for c in candidates if c['source'] != 'alias']
    # A page topic is not an entity name. Prefer explicit entity-level names;
    # retain title/H1 evidence only when no stronger identity source exists.
    explicit = [c for c in candidates if c['source'] in {'schema', 'footer'}]
    if explicit:
        candidates = explicit
    else:
        title_pages = {}
        for c in candidates:
            if c['source'] == 'title':
                title_pages.setdefault(normalize_name(c['value']), set()).add(c['source_url'])
        recurring = {name for name, pages in title_pages.items() if len(pages) >= 2}
        if recurring:
            candidates = [c for c in candidates if normalize_name(c['value']) in recurring]
    if not candidates:
...
def _aliases_field(candidates: List[Dict[str, Any]], canonical_field: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for group in groups.values():
        pages = {candidate.get("source_url") for candidate in group}
        declared = any(
            candidate.get("source") in {"schema", "footer", "alias"}
...
def _official_domain_field(identity_anchor: Dict[str, Any]) -> Dict[str, Any]:
    value = identity_anchor["self_url"]
    return {
        "value": value,
        "candidates": [{"value": value, "source": "schema"}] if value else [],
        "consistent": None,
