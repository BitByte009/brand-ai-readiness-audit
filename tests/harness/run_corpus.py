...
def _evidence_quality(finding: Dict[str, Any], report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    observations = {o['id']: o for o in report.get('evidence_store', {}).get('observations', [])}
    ids = finding.get('observation_ids', [])
    resolves = bool(ids) and all(oid in observations for oid in ids)
    ok = has_text and names_something and has_observed_signal and resolves
...
def score_fixture(fixture_dir: Path) -> Dict[str, Any]:
    ...
    confirmed_fn = expected.get("confirmed_false_negative", expected.get("expected_false_negative"))
    ...
    with FixtureServer(fixture_dir) as server:
        ...
        with patch.object(public_transport, "public_address", fixture_address):
            report = run_audit(url, {"include_evidence": True})
    ...
    total_missing_checks = sorted((expected_findings | structurally_untestable | confirmed_fn_checks) - actual_check_ids)
    ...
    return {
        ...
        "total_missing_checks": total_missing_checks,
        "known_coverage_gaps": sorted((structurally_untestable | confirmed_fn_checks) - actual_check_ids),
        ...
        "report": {k: v for k, v in report.items() if k != "evidence_store"},
    }
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    total_missing = sum(len(r["total_missing_checks"]) for r in results)
    ...
    print("=" * 100)
    print(f"Missing checks including known capability gaps: {total_missing}; unexpected false negatives: {total_fn}")
