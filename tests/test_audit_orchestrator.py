...
def robots_error(origin: str) -> Dict[str, Any]:
    return {"url": origin + "/robots.txt", "status": "error", "http_status": 503, "allow_all": False, "document": {"groups": [], "sitemap": [], "rules": []}}
...
def test_classify_archetype_from_jsonld_local_business():
    obs = make_observation("HTTP_FETCH", "https://x.example/", {"status_code": 200, "html": '<script type="application/ld+json">{"@type":"LocalBusiness"}</script>'})
...
def test_proactive_flags_missing_sameas_for_organization_markup():
    obs = make_observation("HTTP_FETCH", "https://x.example/", {"status_code": 200, "html": '<script type="application/ld+json">{"@type":"Organization","name":"X"}</script>'})
...
def test_proactive_says_nothing_when_sameas_present():
    obs = make_observation("HTTP_FETCH", "https://x.example/", {"status_code": 200, "html": '<script type="application/ld+json">{"@type":"Organization","name":"X","sameAs":["https://en.wikipedia.org/wiki/X"]}</script>'})
    opportunities = proactive.generate_proactive_opportunities({"observations": [obs]}, demoted=[])
...
def test_run_audit_robots_unreachable_yields_scoped_finding_and_valid_report():
    def fetch_should_never_be_called(url):
    ...
    assert any(f["check_id"] == "D-CRAWL-15" and f["severity"] == "high" for f in report["findings"])
    is_valid, errors = validate_report_schema(report)
    assert is_valid, errors
