...
def test_score_allowlist_does_not_force_weak_evidence_to_critical():
    # A check ID alone does not prove a whole-site, high-confidence defect.
    finding = make_finding(check_id="D-CRAWL-01", severity="critical", confidence="low", count=1, total=1)
    scored = sc.score_finding(finding)
    assert scored["severity"] == "high"
    assert not any("critical-floor" in m for m in scored["scoring_trace"]["modifiers_applied"])
...
def test_distribution_guard_never_touches_critical():
    ...
    findings = [
        sc.score_finding(make_finding(check_id="D-CRAWL-01", severity="critical", confidence="high", count=5, total=5)),
...
def test_hostile_severe_issue_weak_evidence_is_demoted_not_asserted():
    ...
    assert len(result["demoted"]) == 1
    assert result["demoted"][0]["severity"] == "high"
