...
def test_d_crawl_07_absence_only_fires_with_orphans():
    ...
    assert not detect_crawl.check_d_crawl_07(store)  # unchecked is not missing
    store["observations"].append(make_observation("SITEMAP", "https://example.com/sitemap.xml", {"status": "missing"}))
...
def test_d_crawl_15_fires_on_robots_5xx():
    ...
    store["observations"][0]["value"]["http_status"] = 503
...
def test_d_render_05_fires_on_large_listing_with_no_pagination():
    ...
    rendered_html = f"<html><body><div class='grid'>{items}</div><button>Load more</button></body></html>"
...
def test_d_extract_02_fires_on_duplicate_title_across_distinct_pages():
    ...
    html_b = page(title="Same Title", body="A different subject with independently useful content. " * 20)
    store = make_store(
        [
