...
| `PAGE_CLASSIFICATION` | zero or one per URL | the page URL | `{page_type: product\|article\|organization\|local_business\|faq\|event\|other}` — LLM instrument, `lib/site_observer/classify.py` |
| `PROBE` | zero or one per URL | the page URL | `{questions: [{id, category: factual\|identity\|engagement, relevant: bool, expects_explicit_statement: bool, answered: bool, answer, evidence_span}], source_text_hash}` — LLM instrument, `lib/site_observer/probe.py` |
