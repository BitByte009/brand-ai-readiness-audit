...
from lib.common.extract import extract_canonical_url, extract_text
...
def check_d_crawl_07(store: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if sitemap and sitemap.get("value", {}).get("status") == "ok":
        entries = sitemap["value"].get("entries", [])
        if entries:
            rotten = [e for e in entries if not e.get("status_code") or e["status_code"] >= 300]
    ...
    elif not sitemap or sitemap.get("value", {}).get("status") == "missing":
...
def check_d_crawl_15(store: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    return [
        make_finding(
            ...
            evidence=f"robots.txt fetch status: {robots['value'].get('status')}",
            observation_ids=[robots["id"]],
