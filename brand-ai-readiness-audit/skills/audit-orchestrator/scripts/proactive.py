...
from lib.common.extract import extract_jsonld  # noqa: E402
...
def _identity_anchor_opportunity(store: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    fetch_observations = [obs for obs in store.get("observations", []) if obs.get("type") in ("HTTP_FETCH", "RENDER")]
    ...
    if has_organization_or_person and not has_same_as:
        return {
            ...
            "opportunity": (
                ...
                "Adding links to the entity's authoritative profiles (official social accounts, "
