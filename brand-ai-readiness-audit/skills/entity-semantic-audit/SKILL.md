...
`references/probe-questions.md`) are consulted only when a field has zero
deterministic candidates:

| Script | Role | CLI |
|---|---|---|
| `scripts/build_entity_profile.py` | Builds the entity fact table (`references/entity-profile-contract.md`) | `python build_entity_profile.py --store <path> [--out <path>]` |
| `scripts/detect_entity.py` | D-ENTITY-01..06 over the profile | `python detect_entity.py --store <path> [--profile <path>] [--out <path>]` |

`scripts/_entity_util.py` is shared, not a public entrypoint (named distinctly
...
## Procedure
1. Read the archetype and load its applicability row from `<marketplace-root>/references/archetype-applicability.md`.
2. Build the entity fact table with per-page, per-field provenance.
3. Run `references/entity-checks.md` in ID order, applicability precondition first.
...
suggested_action`), plus `entity_profile` consumed by `trust-freshness-audit`,
`engagement-audit` and the proactive layer. A proposed base severity and bound
...
record exists — before it fires. See `references/fp-guardrails.md`.
...
## Tests
`<marketplace-root>/tests/test_entity_semantic_audit.py` — unit tests per check,
