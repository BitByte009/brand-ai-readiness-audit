...
from lib.common.extract import extract_jsonld, extract_text
...
def _extract_entity_fact_claims(url: str, oid: str, text: str) -> List[Dict[str, Any]]:
    ...
    if match:
        claims.append(
            _claim("entity_fact", match.group(0), url, oid, key="customer_count", value=normalize_claim_value(match.group(1)))
        )

    match = _EMPLOYEE_COUNT_RE.search(text)
    if match:
        claims.append(
            _claim("entity_fact", match.group(0), url, oid, key="employee_count", value=normalize_claim_value(match.group(1)))
        )
...
def build_claim_table(store: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for url, page in pages.items():
        ...
        claims.extend(_extract_superlative_claims(url, oid, text))
