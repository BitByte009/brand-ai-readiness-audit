...
## Inputs
Observation store plus `entity_profile` (built by `entity-semantic-audit`, used
only for D-TRUST-05's identity-confusion guard). Outbound corroboration only if
the orchestrator's capability matrix reports it available. Exact observation
shapes: `references/store-contract.md`.

## Scripts
Deterministic extraction and comparison throughout — no model call is required
for a complete first implementation (see `references/store-contract.md`'s note
on where an LLM-instrument upgrade is a documented, not required, extension
point):

| Script | Role | CLI |
|---|---|---|
| `scripts/build_claim_table.py` | Builds the claim table (`references/claim-table-contract.md`) | `python build_claim_table.py --store <path> [--out <path>]` |
| `scripts/corroborate.py` | Records one `CLAIM_CORROBORATION` observation from search results (or an honest absent-capability record if none supplied) | `python corroborate.py --claim <path> [--results <path>] [--entity-profile <path>] [--out <path>]` |
| `scripts/detect_trust.py` | D-TRUST-01..06 over the claim table | `python detect_trust.py --store <path> [--claim-table <path>] [--out <path>]` |

`scripts/_trust_util.py` is shared, not a public entrypoint. If `--claim-table`
...
2. Run `references/trust-checks.md` in ID order, applicability precondition first.
3. Corroboration only if capability is present; write a corroboration record.
...
See `references/corroboration-honesty.md`.
...
(`references/corroboration-honesty.md`). Distinguish observed facts from
...
- `<marketplace-root>/lib/` — shared instrumentation, including the common finding
  ...
  never fetches the target site. See PROJECT_CONTEXT.md decision D-6.
...
## Tests
`<marketplace-root>/tests/test_trust_freshness_audit.py` — unit tests per check,
