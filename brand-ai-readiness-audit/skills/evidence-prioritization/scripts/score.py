...
def score_finding(finding: Dict[str, Any], store: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if finding["check_id"] in CRITICAL_ALLOWLIST:
        if final_severity != "critical":
            trace.append(f"critical-floor: total-invisibility check, restored to critical from {final_severity}")
        final_severity = "critical"
    elif final_severity == "critical":
        final_severity = "high"
        trace.append("critical-cap: not on the total-invisibility allowlist, demoted to high")
