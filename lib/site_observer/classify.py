...
def classify_page(html: str) -> str:
    """Prefer explicit semantic types; abstain when the page role is uncertain."""
    from bs4 import BeautifulSoup
    soup = BeautifulSoup(html or '', 'html.parser')
    types = {t for node in extract_jsonld(html) for t in schema_type_names(node.get('@type'))}
    for kind, label in [('Product', 'product'), ('Article', 'article'), ('BlogPosting', 'article'),
                        ('NewsArticle', 'article'), ('ContactPage', 'contact'), ('AboutPage', 'about'),
                        ('FAQPage', 'faq'), ('Event', 'event')]:
        if kind in types:
            return label
    if soup.find('article'):
        return 'article'
    if soup.select('[itemtype$="/Product"]'):
        return 'product'
    return 'other'
